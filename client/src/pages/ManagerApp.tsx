...
import {
  BarChart3, Check, ClipboardList, ChevronRight, HeartHandshake, LayoutDashboard,
  MapPin, Package, Pencil, Plus, Settings, ShoppingCart, Truck, Users, WalletCards, X, UserRound, Store, Zap
} from "lucide-react";
import { supabase, hasSupabase } from "../lib/supabase";
import { useOrders, useProducts, useSettings } from "../lib/hooks";
import {
  money, moneyFromCents, orderTotal, orderLabel, orderName, payLabel, dateBR,
  uploadToBucket, openWhatsApp, ALL_STATUSES, CATNAMES, statusIdx
} from "../lib/helpers";
import { Button, Header, Metric } from "../ui/components";
import { IMG } from "../data/mock";
import { triggerInstall } from "./Home";

export default function ManagerApp({ tab, setTab, onSignOut }: {
  tab: string; setTab: (s: string) => void; onSignOut: () => void;
}) {
  ...
  useEffect(() => {
    if (hasSupabase) supabase.from("order_items").select("*").then(({ data }) => setAllItems(data || []));
  }, [orders.length]);

  useEffect(() => {
    if (exp && hasSupabase) supabase.from("order_items").select("*").eq("order_id", exp).then(({ data }) => setExpItems(data || []));
    else setExpItems([]);
  }, [exp]);
  ...
  const revenue = allItems.reduce((s, it) => s + (it.unit_price_cents || 0) * it.qty, 0) / 100;
  const cost = allItems.reduce((s, it) => s + ((prodById(it.product_id)?.cost_cents || 0) * it.qty), 0) / 100;
  const profit = revenue - cost;
  const margin = revenue > 0 ? (profit / revenue) * 100 : 0;
  const byProd = (() => {
    const m: Record<string, { name: string; qty: number; profit: number; cat: number }> = {};
    allItems.forEach(it => {
      const p = prodById(it.product_id);
      const key = it.product_name;
      m[key] = m[key] || { name: key, qty: 0, profit: 0, cat: p?.category_id || 0 };
      m[key].qty += it.qty;
      m[key].profit += ((it.unit_price_cents - (p?.cost_cents || 0)) / 100) * it.qty;
    });
  ...
  const byCat = (() => {
    ...
    byProd.forEach(b => { m[b.cat] = (m[b.cat] || 0) + b.profit + 0; });
    return m;
  })();
  ...
  const clients = (() => {
    ...
    orders.forEach(o => {
      ...
      m[key] = m[key] || { name: o.customer_name, phone: o.customer_phone || "", count: 0, total: 0, last: o.created_at };
      m[key].count++;
      m[key].total += orderTotal(o);
      if ((o.created_at || "") > (m[key].last || "")) m[key].last = o.created_at;
    });
  ...
  const araraquara = orders.filter(o => o.city === "Araraquara" || o.delivery_mode === "ENCOMENDA");

  const uploadPhoto = async (photo: { file: File; preview: string } | null, prefix: string) => {
    if (!photo) return null;
    try { return await uploadToBucket("product-photos", `${prefix}-${Date.now()}.jpg`, photo.file); }
    catch { toast.error("Falha no upload da foto."); return null; }
  };

  const saveEdit = async () => {
    const uploaded = await uploadPhoto(editPhoto, edit.isNew ? "prod" : `prod-${edit.id}`);
    const payload = {
    ...
    if (edit.isNew) { await insertProd(payload); toast.success("Produto criado"); }
    else { await patchProd(edit.id, payload); toast.success("Produto atualizado"); }
    setEdit(null); setEditPhoto(null);
  };
    ...
    <div className="app-shell manager">
      ...
      <main className="page manager-page">
        {tab === "dashboard" && (
          <>
            <div className="manager-welcome">
              <div><span>{new Date().toLocaleDateString("pt-BR", { weekday: "long", day: "numeric", month: "long" })}</span><h1>Bom dia, gestor.</h1></div>
            ...
            {canInstall && !installed && (
              <Button
                ...
                onClick={async () => {
                  ...
                  if (ok) {
                    toast.success("App instalado! Agora você pode abrir como app nativo.");
                    setInstalled(true);
                    setCanInstall(false);
                  }
                }}
            ...
            {installed && (
              <div style={{ background: "#e2f0e7", padding: "10px 14px", borderRadius: 10, fontSize: 12, color: "#235842", marginBottom: 14 }}>
            ...
            <div className="metric-grid">
              ...
              <Metric label="Lucro (itens)" value={money(profit)} trend={`${margin.toFixed(1)}% margem`} />
              <Metric label="Estoque baixo" value={String(lowStock)} trend="ver produtos" alert={lowStock > 0} />
            </div>
            <div className="dashboard-card">
              <div className="section-head"><h2>Vendas por semana</h2><span className="muted">por dia</span></div>
              <div className="bars">{week.map((w, i) => <i key={i} style={{ height: `${Math.max(w.h, 6)}%` }} />)}</div>
              <div className="days">{week.map(w => <span key={w.d}>{w.d}</span>)}</div>
            </div>
            <div className="split-cards">
              <div><X /><b>{urgentCount} urgentes</b><span>pedidos para olhar</span></div>
              <div><Truck /><b>{pendCount} pendência(s)</b><span>de entrega</span></div>
            </div>
          </>
        )}
        ...
        {tab === "products" && (
          <>
            <div className="list-head">
              <div><span className="eyebrow">gestão</span><h1>Produtos</h1><p>{prods.filter(p => p.active !== false).length} ativos</p></div>
              <Button onClick={() => { setEdit({ isNew: true, name: "", description: "", price: "", cost: "", stock: "0", image_url: IMG.doces, category_id: 1, active: true }); setEditPhoto(null); }}>
            ...
            <div className="manager-list">
              {prods.map(p => (
                <div className="manager-row" key={p.id}>
                  ...
                  <div><b>{p.name}{p.active === false ? " (inativo)" : ""}</b><span>{p.stock} un · {CATNAMES[p.category_id] || "Outros"}</span></div>
                  <strong>{moneyFromCents(p.price_cents)}</strong>
                  <button className="icon-btn" style={{ width: 32, height: 32 }} onClick={() => { setEdit({ ...p, price: (p.price_cents / 100).toFixed(2), cost: (p.cost_cents / 100).toFixed(2), stock: String(p.stock) }); setEditPhoto(null); }}><Pencil size={14} /></button>
                  <button className="icon-btn" style={{ width: 32, height: 32, color: "#bd463b" }} onClick={() => { if (window.confirm(`Desativar ${p.name}?`)) { patchProd(p.id, { active: false }); toast.success("Desativado"); } }}><X size={14} /></button>
                </div>
              ))}
            </div>
            {edit && (
              <div className="settings-card" style={{ marginTop: 14 }}>
                ...
                <div className="crop-preview"><img src={editPhoto?.preview || edit.image_url} /><span>foto atual / nova foto</span></div>
                <div className="proof-pickers">
                  <label><CameraIcon onPick={(f, p) => setEditPhoto({ file: f, preview: p })} /></label>
                </div>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <input type="checkbox" style={{ width: "auto", height: "auto" }} checked={edit.active !== false} onChange={e => setEdit({ ...edit, active: e.target.checked })} />
                  Ativo no catálogo
                </label>
        ...
        {tab === "stock" && (
          <div className="manager-list">
            {prods.filter(p => p.active !== false).map(p => (
              <div className="manager-row" key={p.id}>
                <div className="mini-avatar">{p.name[0]}</div>
                <div><b>{p.name}</b><span>mínimo 10 unidades</span></div>
        ...
        {tab === "orders" && (
          <>
            <div className="list-head">
              <div><span className="eyebrow">gestão</span><h1>Pedidos</h1><p>{filteredOrders.length} de {orders.length}</p></div>
              <div style={{ display: "flex", gap: 6 }}>
                <select style={{ height: 38, fontSize: 12 }} value={fStatus} onChange={e => setFStatus(e.target.value)}>
                  <option value="TODOS">Todos status</option>
                  {ALL_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <button className={`btn ${fUrgent ? "btn-primary" : "btn-soft"}`} style={{ width: "auto", minHeight: 38, padding: "0 10px", fontSize: 11 }} onClick={() => setFUrgent(v => !v)}>🚨</button>
              </div>
            </div>
            <div className="manager-list">
              {filteredOrders.map(o => (
                <div key={o.id}>
                  <div className="manager-row">
                    ...
                    <div onClick={() => setExp(exp === o.id ? null : o.id)} style={{ cursor: "pointer" }}>
                      <b>{orderLabel(o)} · {orderName(o)}{o.urgent ? " 🚨" : ""}</b>
                      <span>{o.city} · {payLabel(o)} · {money(orderTotal(o))}</span>
                    </div>
                    <select style={{ height: 34, fontSize: 11, width: 160 }} value={o.status} onChange={e => { patch(o.id, { status: e.target.value }); toast.success(`Status → ${e.target.value}`); }}>
                      {ALL_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  {exp === o.id && (
                    <div className="settings-card" style={{ margin: "6px 0 10px" }}>
                      <span className="eyebrow">itens do pedido</span>
                      {expItems.length === 0 && <p className="muted">Sem itens detalhados.</p>}
                      {expItems.map(it => <p key={it.id} style={{ fontSize: 12, margin: "4px 0" }}>{it.qty}x {it.product_name} — {moneyFromCents(it.unit_price_cents * it.qty)}</p>)}
                      {o.proof_url && (<div className="proof-preview"><img src={o.proof_url} /><span>comprovante do cliente</span></div>)}
                      ...
                      }}><HeartHandshake size={16} /> Falar com cliente</Button>
                    </div>
                  )}
                </div>
              ))}
        ...
        {tab === "clients" && (
          <div className="manager-list">
            {clients.length === 0 && <p className="muted">Sem clientes ainda.</p>}
            {clients.map((c: any, i: number) => (
              <div className="manager-row" key={i}>
                <div className="mini-avatar">{(c.name || "C")[0]}</div>
                <div><b>{c.name}</b><span>{c.phone || "sem telefone"} · {c.count} pedido(s) · último {dateBR(c.last)}</span></div>
        ...
        {tab === "reports" && (
          <>
            <div className="report-big">
              <span>Faturamento (itens)</span><b>{money(revenue)}</b>
              <small>Custo {money(cost)} · Lucro {money(profit)} · Margem {margin.toFixed(1)}%</small>
            </div>
            <div className="report-card">
              <b>Mais vendidos (qtd)</b>
              {topSold.map(t => <span key={t.name}>{t.name} <strong>{t.qty}x</strong></span>)}
            </div>
            <div className="report-card">
              <b>Mais lucrativos (R$)</b>
              {topProfit.map(t => <span key={t.name}>{t.name} <strong>{money(t.profit)}</strong></span>)}
            </div>
            <div className="report-card">
              <b>Lucro por categoria</b>
              {Object.entries(byCat).map(([cat, v]) => <span key={cat}>{CATNAMES[Number(cat)] || "Outros"} <strong>{money(v)}</strong></span>)}
            </div>
          </>
        )}
            ...
            {regions.map(r => (
              <div className="route-card" key={r} style={{ marginBottom: 10 }}>
                <div className="route-title">
                  <Truck /><div><b>Rota {r}</b><span>{ready.filter(o => (o.region || "Centro") === r).length} prontos</span></div>
        ...
        {tab === "araraquara" && (
          <div className="settings-card">
            ...
            <h1>Araraquara</h1>
            <div className="date-box"><MapPin /><b>{dateBR(settings?.araraquara_next_date) || "15/08/2026"}</b><span>próxima encomenda</span></div>
            {araraquara.map(o => (
              <div className="manager-row" key={o.id} style={{ marginBottom: 6 }}>
                <div className="mini-avatar">{orderName(o)[0]}</div>
                <div><b>{orderName(o)}</b><span>{orderLabel(o)} · {payLabel(o)}</span></div>
                <strong>{money(orderTotal(o))}</strong>
              </div>
            ))}
            {araraquara.length === 0 && <p className="muted">Nenhuma encomenda aberta.</p>}
            <Button onClick={() => setTab("settings")}>Alterar data <Pencil size={16} /></Button>
          </div>
        )}
        ...
        {tab === "settings" && form && (
          <div className="settings-card">
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" style={{ width: "auto", height: "auto" }} checked={form.pickup_enabled !== false} onChange={e => setForm({ ...form, pickup_enabled: e.target.checked })} />
              Retirada habilitada
            </label>
            <Button onClick={async () => { await saveSettings(form); toast.success("Configurações salvas"); }}>Salvar configurações <Check size={16} /></Button>
          </div>
        )}
      </main>
      <nav className="manager-nav">
        {items.map(i => (
          <button key={i.id} className={tab === i.id ? "active" : ""} onClick={() => setTab(i.id)}>{i.icon}<span>{i.label}</span></button>
        ))}
        <button onClick={onSignOut}><X /><span>Sair</span></button>
      </nav>
    </div>
  );
}

function CameraIcon({ onPick }: { onPick: (f: File, preview: string) => void }) {
  return (
    <>
