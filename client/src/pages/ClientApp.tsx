...
export default function ClientApp() {
  ...
  const cartTotal = cartItems.reduce((s, { p, q }) => s + (p?.price_cents || 0) * q, 0) / 100;

  const add = (id: string) => setCart(c => ({ ...c, [id]: (c[id] || 0) + 1 }));
  const remove = (id: string) => setCart(c => ({ ...c, [id]: Math.max(0, (c[id] || 0) - 1) }));
  const clear = (id: string) => setCart(c => { const n = { ...c }; delete n[id]; return n; });

  const go = (s: string) => { setScreen(s); window.scrollTo({ top: 0, behavior: "smooth" }); };
      ...
      <div className="app-shell">
        ...
        <main className="page">
          ...
          <div className="product-detail">
            ...
            <Button onClick={() => { add(selected.id); toast.success("Produto adicionado ao carrinho"); }}>
              Adicionar ao carrinho <ArrowRight size={17} />
            </Button>
            <WhatsAppBtn number={settings?.whatsapp_number} text={`Olá, Valdir! Tenho uma dúvida sobre ${selected.name}.`} />
          </div>
        </main>
      </div>
    );
    ...
    return (
      <div className="app-shell">
        ...
        <main className="page">
          ...
          <h1>Seu carrinho</h1>
          {cartItems.length === 0 ? (
            <Empty title="Seu carrinho está vazio" text="Escolha um doce para começar." action={() => go("home")} />
          ) : (
            <>
                  ...
                  <div className="cart-row" key={p.id}>
                    ...
                    <div>
                      ...
                      <div className="qty">
                        ...
                        <button className="trash" title="Remover" onClick={() => clear(p.id)}><Trash2 size={15} /></button>
                      </div>
                    </div>
              ...
              <div className="summary">
                <span>Subtotal</span><b>{money(cartTotal)}</b>
                <span>Entrega</span><b className="muted">a combinar</b>
                <hr />
                <strong>Total</strong><strong className="total">{money(cartTotal)}</strong>
              </div>
      ...
      <CheckoutPage
        ...
        decrementStock={async (items: any[]) => {
          ...
          for (const { p, q } of items) {
            await supabase.from("products").update({ stock: Math.max(0, (p.stock || 0) - q) }).eq("id", p.id);
          }
        }}
      />
    );
  ...
  const filtered = products.filter((p: any) => {
    ...
    return (category === "Todos" || catName === category) && p.name.toLowerCase().includes(query.toLowerCase());
  });
    ...
    <div className="app-shell">
      <header className="client-hero">
        <div className="hero-top">
          ...
          <button className="account-btn" onClick={() => go("account")} aria-label="Minha conta">
            <UserRound size={19} />
          </button>
        </div>
      ...
      <main className="page catalog-page" id="main-content">
        ...
        <div className="catalog-actions">
          <WhatsAppBtn number={settings?.whatsapp_number} />
          <button className="install" onClick={() => toast.success("No celular, use 'Adicionar à tela inicial'")}>
        ...
        <div className="section-head">
          ...
          <button className="link-btn" onClick={() => setCategory("Todos")}>ver tudo <ArrowRight size={15} /></button>
        </div>
        <div className="chips">
          {cats.map(c => (
...
function CheckoutPage({ cartItems, cartTotal, step, setStep, settings, customer, uid, onBack, onDone, decrementStock }: any) {
  ...
  const pct = payment === "PIX" ? Number(settings?.entry_pct_pix ?? 0.5) : Number(settings?.entry_pct_card ?? 0.5);
    ...
    try {
      ...
      if (hasSupabase) {
        ...
        const items = cartItems.map(({ p, q }: any) => ({
          order_id: order.id, product_id: p.id, product_name: p.name, qty: q, unit_price_cents: p.price_cents
        }));
      ...
      onDone({
        id: orderNumber, dbId: savedId, accessCode, payment, delivery, city, region, urgent,
        customer: name || "Visitante", phone, payFull, confirmed: false, proof: null, total: cartTotal
      });
    } catch (err: any) {
        ...
        {step === 1 && (
          <>
            ...
            <div className="urgency">
              ...
              <button className={urgent ? "urgent chosen" : ""} onClick={() => setUrgent(true)}>🚨 URGENTE — preciso o quanto antes</button>
              <small>Urgente sinaliza prioridade ao vendedor, não promete entrega imediata.</small>
            </div>
            <Button onClick={() => setStep(2)}>Ir para pagamento <ArrowRight size={17} /></Button>
          </>
        )}
        {step === 2 && (
          <>
            ...
            <div className="pay-options">
              {["PIX", "Cartão", "Dinheiro"].map(p => (
                <button className={payment === p ? "chosen" : ""} key={p} onClick={() => { setPayment(p); setPayFull(false); }}>
                  {p === "PIX" ? <WalletCards /> : p === "Cartão" ? <CreditCard /> : <DollarSign />}
                  <b>{p}</b><span>{p === "Dinheiro" ? "a combinar" : "escolha abaixo"}</span>
                </button>
              ))}
            </div>
            {payment !== "Dinheiro" && (
              <div className="choice-row" style={{ marginTop: 20 }}>
                <button className={!payFull ? "chosen" : ""} onClick={() => setPayFull(false)}>
                  <b>Entrada {Math.round(pct * 100)}%</b><span>{money(entry)} agora, {money(cartTotal - entry)} na entrega</span>
                </button>
                <button className={payFull ? "chosen" : ""} onClick={() => setPayFull(true)}>
                  <b>Pagar integral</b><span>{money(cartTotal)} agora</span>
                </button>
              </div>
            )}
            <div className="summary">
              <span>Total do pedido</span><b>{money(cartTotal)}</b>
              <span>A pagar agora</span><b>{payment === "Dinheiro" ? "—" : money(amountToPay)}</b>
              {needsEntry && (<><span>Saldo na entrega</span><b>{money(cartTotal - entry)}</b></>)}
            </div>
            {payment === "PIX" && (
              <div className="pix-box">
                <span>chave PIX</span><b>{pixKey}</b><small>Titular: {pixHolder}</small>
                <button className={pixCopied ? "copied" : ""} onClick={() => { navigator.clipboard?.writeText(pixKey); setPixCopied(true); }}>
                  {pixCopied ? <><Check size={15} /> chave copiada</> : <><Copy size={15} /> copiar chave PIX</>}
                </button>
                <small>Após pagar, volte ao status do pedido para enviar o comprovante.</small>
              </div>
            )}
            {payment === "Cartão" && (
              <div className="pix-box"><span>cartão · {payFull ? "integral" : `entrada ${Math.round(pct * 100)}%`}</span><small>Os dados do cartão serão combinados com Valdir após a confirmação.</small></div>
            )}
            <Button onClick={() => setStep(3)}>Revisar pedido <ArrowRight size={17} /></Button>
          </>
        )}
        {step === 3 && (
          <>
            ...
            <div className="summary">
              {cartItems.map(({ p, q }: any) => (<span key={p.id}>{q}x {p.name} — {money(p.price_cents * q / 100)}</span>))}
              <hr />
              <strong>Total</strong><strong className="total">{money(cartTotal)}</strong>
              {payment !== "Dinheiro" && (<><strong>A pagar agora</strong><strong className="total">{money(amountToPay)}</strong></>)}
            </div>
            <div className="pix-box">
...
function StatusView({ order, setOrder, settings, go }: any) {
  ...
  const proofNeeded = needsEntry && !order.confirmed && !order.payment_confirmed;
  ...
  const sendProof = async () => {
    ...
    try {
      if (hasSupabase && order.accessCode && proofFile) {
        url = await uploadToBucket("proofs", `${order.accessCode}-${Date.now()}.jpg`, proofFile);
        await supabase.rpc("attach_proof", { code: order.accessCode, proof_url: url });
      }
    ...
    if (!openWhatsApp(settings?.whatsapp_number, msg)) toast.success("Comprovante enviado (simulado)", { description: msg });
  };
      ...
      <main className="page success-page">
        ...
        {order.accessCode && (
          <div style={{ background: "#e2f0e7", padding: "12px", borderRadius: "10px", margin: "10px 0", textAlign: "center" }}>
            <strong style={{ color: "#235842", fontSize: "14px" }}>Código de acesso: <span style={{ fontSize: "18px" }}>{order.accessCode}</span></strong>
            <p style={{ margin: "5px 0 0", fontSize: "11px", color: "#66564e" }}>Guarde para acompanhar depois, em Conta → Acompanhar por código.</p>
          </div>
        )}
        <h1>{sent ? "Comprovante enviado." : "Pedido recebido com carinho."}</h1>
        <div className="order-status">
          <b>{order.payment_confirmed || order.confirmed ? "PAGAMENTO CONFIRMADO" : isCash ? "PEDIDO CONFIRMADO" : "AGUARDANDO CONFIRMAÇÃO DO PAGAMENTO"}</b>
          <span>Cliente: {order.customer}</span>
          <div className="status-summary">
            <span>Total</span><strong>{money(total)}</strong>
            <span>{order.payFull && !isCash ? "PAGO" : needsEntry ? "Entrada" : "A pagar"}</span>
            <strong>{isCash ? "—" : money(needsEntry ? entry : total)}</strong>
            {needsEntry && (<><span>Saldo</span><strong>{money(entry)}</strong></>)}
          </div>
          <div className="status-line">
            {STATUS_SEQ.map((s, i) => (<i key={s} className={i <= idx ? "on" : ""} />))}
          </div>
        ...
        {proofNeeded && !sent && (
          <div className="proof-card">
            <div>
              <span className="eyebrow">{order.payFull ? "pagamento integral" : "entrada"} · {order.payment}</span>
              <h2>Envie o comprovante</h2>
              <p>Escolha uma foto da câmera ou da galeria. Você verá a imagem antes de enviar.</p>
            </div>
            <div className="proof-pickers">
              <label><Camera size={18} /> Tirar foto<input type="file" accept="image/*" capture="environment" onChange={e => handleFile(e.target.files?.[0])} /></label>
              <label><ImageIcon size={18} /> Escolher da galeria<input type="file" accept="image/*" onChange={e => handleFile(e.target.files?.[0])} /></label>
            </div>
            {proofPreview && (
              <div className="proof-preview">
                ...
                <Button variant="whatsapp" onClick={sendProof} disabled={sending}>
                  <HeartHandshake size={17} /> {sending ? "Enviando..." : "Enviar comprovante pelo WhatsApp"}
                </Button>
              </div>
            )}
          </div>
        ...
        {!proofNeeded && isCash && (
          <div className="cash-note">
            <DollarSign size={20} />
            <b>Pagamento: dinheiro na {order.delivery === "Retirada" ? "retirada" : "entrega"}</b>
            <span>Não é necessário enviar comprovante.</span>
          </div>
        )}
        <WhatsAppBtn number={settings?.whatsapp_number} text={`Pedido ${order.id} · ${order.customer}`} />
...
function TrackPage({ settings, go }: any) {
  ...
  const lookup = async () => {
    if (!hasSupabase) { toast.error("Disponível apenas com Supabase configurado."); return; }
    const { data } = await supabase.rpc("get_order_by_access_code", { code: code.trim().toUpperCase() });
  ...
  return (
    <div className="app-shell">
      <Header title="Acompanhar pedido" back onBack={() => go("home")} onLogo={() => go("home")} />
      <main className="page">
        {!order ? (
          <>
            <h1>Tem um código?</h1>
            <p className="muted">Digite o código de acesso que apareceu ao finalizar o pedido.</p>
            <label>Código<input value={code} onChange={e => setCode(e.target.value)} placeholder="Ex.: A1B2C3" /></label>
            <Button onClick={lookup}>Ver status <ArrowRight size={16} /></Button>
          </>
        ) : (
          <StatusView order={order} setOrder={setOrder} settings={settings} go={go} />
        )}
      </main>
    </div>
  );
